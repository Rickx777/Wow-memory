...
/*void main(){
  ...
  //add_box(new,200,200,0,40,40,40);
  //print_matrix(new);

  //add_sphere(new, 250, 250, 0, 150, 0.01);

  add_torus(new,250,250,0,60,30,0.01);
  matrix_mult(rot,new);
...
void add_sphere( struct matrix * edges, 
  ...
  for (c = 0; (c + 1) < tmp->lastcol; c++){
    add_edge(edges,tmp->m[0][c],tmp->m[1][c],tmp->m[2][c],
	     tmp->m[0][c+1], tmp->m[1][c+1], tmp->m[2][c+1]);
  }
  free_matrix(tmp);
}
...
struct matrix * generate_sphere(double cx, double cy, double cz,
  ...
  double rot,cir,x,y,z;
  for (rot = 0; rot < 1.0001; rot += step){
    for (cir = 0; cir < 1.0001; cir += step){
      x = r * cos(2 * M_PI * cir) + cx;
      y = r * sin(2 * M_PI * cir) * cos(2 * M_PI * rot) + cy;
      z = r * sin(2 * M_PI * cir) * sin(2 * M_PI * rot) + cz;
...
struct matrix * generate_torus( double cx, double cy, double cz,
  ...
  for (rot = 0; rot < 1.0001; rot += step){
    for (cir = 0; cir < 1.0001; cir += step){
      ...
      z = -sin(2 * M_PI * rot) * (r1 * cos( 2 * M_PI * cir) + r2) + cz;
      add_point(edges,x,y,z);
    }
  }
  return edges;
